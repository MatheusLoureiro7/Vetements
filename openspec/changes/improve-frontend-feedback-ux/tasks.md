...
- [ ] 2.1 Adicionar Var de UI (aberta/fechada) para a sidebar e um
  botão de menu na topbar visível abaixo do breakpoint de 768px, em
  `vetements/components/shell.py`
- [ ] 2.2 Colapsar a sidebar por padrão abaixo de 768px e mostrá-la ao
...
- [ ] 3.1 Adicionar `is_submitting: bool = False` a `AuthState`, setar
  `True` no início de `login()` e `False` em todo caminho de saída
  (sucesso ou erro), e retornar cedo se já `True`
- [ ] 3.2 Em `pages/login.py`, usar `AuthState.is_submitting` para
...
- [ ] 5.1 Adicionar `is_submitting: bool = False` e `success: str` a
  `CustomersState`, com a mesma lógica de 4.1/4.2 em
  `create_customer()`
- [ ] 5.2 Adicionar `is_loading_page: bool = True` a `CustomersState`,
  setado `False` ao final de `load()`
- [ ] 5.3 Em `pages/customers.py`, usar os Vars de 5.1–5.2: botão
...
- [ ] 6.1 Adicionar `is_submitting: bool = False` a `SalesState`, com
  a mesma lógica de 3.1 em `confirm_sale()`
- [ ] 6.2 Adicionar `is_loading_page: bool = True` a `SalesState`,
  setado `False` ao final de `load()`
- [ ] 6.3 Em `pages/sales.py`, usar os Vars de 6.1–6.2: botão
...
- [ ] 7.1 Adicionar `is_loading_page: bool = True` a `InventoryState`,
...
- [ ] 7.2 Adicionar `is_loading_page: bool = True` a `DashboardState`,
...
- [ ] 8.1 Rodar `pytest` e confirmar que a suíte existente continua
  passando
- [ ] 8.2 Rodar `reflex compile --dry` e confirmar compilação sem erro
- [ ] 8.3 Subir o app (`reflex run --env prod --single-port`) e, pelo
